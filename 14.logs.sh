...
SCRIPT_NAME=$(echo $0 | cut -d "." -f1)  # $0 replace with script name
LOG_FILE="$LOGS_FOLDER/$SCRIPT_NAME.log"

mkdir -p $LOGS_FOLDER  # -p checks whether folder created or not
echo "Script started executing at: $(date)" | tee -a $LOG_FILE  # Basic log on the screen it displays both in file and screen a- append 
